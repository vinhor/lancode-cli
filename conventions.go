...
func javaScriptConventions(json JSONData, conceptIndex int) {
	...
	keyword := color.New(color.FgRed).SprintFunc()
	specialOperator := color.New(color.FgMagenta).SprintFunc()
	name := color.New(color.FgHiBlue).SprintFunc()
	number := color.New(color.FgHiMagenta).SprintFunc()
	...
	specialName := color.New(color.FgYellow).SprintFunc()
	...
	case "function":
		...
		fmt.Printf("%s\n", bracket("}"))
	case "class":
		fmt.Printf("%s %s %s\n", keyword("class"), specialName("SeniorEngineer"), bracket("{"))
		...
		fmt.Printf("%s\n", bracket("}"))
	case "type":
		fmt.Printf("%s %s = %s %s %s\n", keyword("type"), specialName("EngineersSeniority"), stringColor("Junior"), specialOperator("|"), stringColor("Senior"))
	case "interface":
		fmt.Printf("%s %s %s\n", keyword("interface"), specialName("ColorRgb"), bracket("{"))
		fmt.Printf("  %s%s %s\n", name("red"), operator(":"), specialName("number"))
		fmt.Printf("  %s%s %s\n", name("green"), operator(":"), specialName("number"))
		fmt.Printf("  %s%s %s\n", name("blue"), operator(":"), specialName("number"))
		fmt.Printf("%s\n", bracket("}"))
	}
}
